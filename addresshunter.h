...
//redefine UNICODE_STRING struct
...
UINT64 GetNtdll() {
    ULONG_PTR ntdlldll, val1, val2, val3;
    USHORT usCounter;

   
    ntdlldll = __readgsqword( 0x60 );

    ntdlldll = (ULONG_PTR)((_PPEB)ntdlldll)->pLdr;
    val1 = (ULONG_PTR)((PPEB_LDR_DATA)ntdlldll)->InMemoryOrderModuleList.Flink;
    while( val1 ) {
        val2 = (ULONG_PTR)((PLDR_DATA_TABLE_ENTRY)val1)->BaseDllName.pBuffer;
        usCounter = ((PLDR_DATA_TABLE_ENTRY)val1)->BaseDllName.Length;
        val3 = 0;

        //calculate the hash of ndll.dll
        do {
            val3 = ror13( (DWORD)val3 );
            if( *((BYTE *)val2) >= 'a' )
                val3 += *((BYTE *)val2) - 0x20;
        ...
        if( (DWORD)val3 == NTDLLDLL_HASH ) {
            ...
            ntdlldll = (ULONG_PTR)((PLDR_DATA_TABLE_ENTRY)val1)->DllBase;
            return ntdlldll;
        }
        val1 = DEREF( val1 );
    }
